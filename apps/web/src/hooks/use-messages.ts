...
import { mapMessage, mapMessageThread, type MessageThreadWire, type MessageWire } from '@/lib/api/contracts';

export function useMessageThreads() {
  return useQuery({
    queryKey: ['messages', 'threads'],
    queryFn: async () => {
      const wire = await apiFetch<{ threads: MessageThreadWire[] }>('/messages/threads');
      return wire.threads.map(mapMessageThread);
    },
