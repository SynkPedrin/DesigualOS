...
export default function AnalyticsPage() {
  const { data: executions, isPending, isError, refetch } = useExecutions();
  ...
  const avgDurationByAgent = useMemo(() => {
    ...
    for (const execution of executions) {
      if (execution.status !== 'completed' || !execution.completedAt || !execution.startedAt) continue;
    ...
    <div>
      ...
      {isError ? (
        <div className="space-y-4">
          <EmptyState
            icon={BarChart3}
            title="Não conseguimos carregar os dados de analytics."
